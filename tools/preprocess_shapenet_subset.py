...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Preprocess and split downloaded ShapeNet subset")
    parser.add_argument(
        "--input-root",
        default="data/shapenet_subsets/furniture_plus_display_cap1000",
        help="Downloaded subset root",
    )
    parser.add_argument(
        "--output-root",
        default="artifacts/shapenet_furniture_plus_display_cap1000_preprocessed",
        help="Preprocessed output root",
    )
    parser.add_argument("--train-ratio", type=float, default=0.7)
    parser.add_argument("--val-ratio", type=float, default=0.15)
    parser.add_argument("--test-ratio", type=float, default=0.15)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--min-vertices", type=int, default=16)
    parser.add_argument("--min-faces", type=int, default=16)
    parser.add_argument(
        "--enable-self-intersection-check",
        action="store_true",
        help="Enable Open3D self-intersection checks if available",
    )
...
def main() -> None:
    ...
    print("ShapeNet subset preprocessing complete")
